...
5. [Modelling & Evaluation](#5-modelling--evaluation)  
   ...
   5.6 [Image Based Weather Severity Classifier](#56-image-based-weather-severity-classifier-supporting-model)
...
While this project successfully demonstrates end-to-end predictive analytics workflows, several limitations should be noted:

- **Synthetic data assumptions**  
  The bookings, cancellations, and weather datasets are synthetically generated.  
  Although designed to reflect realistic patterns, they cannot capture the full complexity and unpredictability of real-world customer behaviour.

- **Forecast horizon uncertainty**  
  The regression model is most informative for **short- to medium-term planning**.  
  Forecast uncertainty increases further into the future, especially where lag-based features dominate.

- **Cancellation behaviour complexity**  
  Cancellations are influenced by many unobserved factors (personal circumstances, transport issues, health), which are not represented in the dataset.  
  As a result, the classification model is best used as a **risk-ranking tool**, not a definitive decision-maker.

- **Image-based weather classification scope**  
  The Weather Severity Classifier is intended as a **supporting, illustrative tool**.  
  It does not replace professional judgement or official weather forecasts and is not integrated directly into the forecasting pipelines.

- **Prototype-level deployment**  
  This application is designed as a **decision-support prototype**.  
  A production system would require live data feeds, continuous retraining, monitoring, and stronger validation against real outcomes.

These limitations are acknowledged transparently and are consistent with early-stage analytics prototypes used to explore feasibility, value, and next steps.


---

 ### 5.6 Image-Based Weather Severity Classifier (Supporting Model)
